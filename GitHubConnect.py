...
This is the first test release of my module `pyGitConnect`. That was created for this project for simplify working with GitHub as a cloud storage

You can see the actual release of `pyGitConnect` [here](https://github.com/tankalxat34/pyGitConnect)
"""
import getpass
...
class File:
    ...
    def __init__(self, user_class, repository: str, branch: str, path: str):
        ...
        self.response = requests.get(self.url, auth=self.user.auth).json()

    def get(self):
        try:
            return requests.get(self.response["download_url"], auth=self.user.auth).content
        except KeyError:
            raise Exceptions.FileNotFound(self.path)
